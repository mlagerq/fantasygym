...
for team_id, team_name in team_options.items():
    ...
    WebDriverWait(driver, 10).until(
        lambda d: [option.text.strip() for option in Select(d.find_element(By.ID, "gymnast_filter")).options if option.text.strip()] != initial_gymnast_options
    )
...
# Save to CSV
df.to_csv("road_to_nationals.csv", index=False)
